...
def adam_opt(
    ...
    if scheduler:
        learning_rate = optax.cosine_decay_schedule(
            init_value=learning_rate, decay_steps=steps, alpha=0.0
        )
    ...
    def step(params, state, args, lkey):
        if loss_key:
            loss_value, grads = jax.value_and_grad(loss_fn)(params, *args, lkey)
        else:
    ...
    for i in pbar:
        ...
        if callable(args_fn):
            if key is not None:
                key, skey, lkey = jax.random.split(key, num=3)
                args = args_fn(skey)
            else:
                args = args_fn()
        else:
        ...
        cur_loss, params_new, state_new = step(params, state, args, lkey)
